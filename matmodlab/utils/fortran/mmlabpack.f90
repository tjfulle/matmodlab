...
contains
  ...
  subroutine F_from_E(k, E, F)
    ...
    if (k == 0) then
       ...
       U = powm(k * eps + I, one / k)
    end if
    F = reshape(matmul(R, U), shape(F))
  end subroutine F_from_E
  ...
  function inv6(a)
    ...
    real(kind=8) :: inv6(6)
    real(kind=8), intent(in)  :: a(6)
    real(kind=8) :: w(3,3), wi(3,3)
    w = asmat(a)
    wi = inv(w)
    call asarray(wi, 6, inv6)
  end function inv6
