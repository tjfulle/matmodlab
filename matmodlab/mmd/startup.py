...
commands = ('build', 'clean', 'examples', 'helper', 'run', 'test', 'view', 'ipynb', 'notebook')
...
The mml commands are:
  ...
  examples Create a directory of examples
...
def main(argv=None):
    ...
    if x != 'run':
        ...
        if x == 'view':
            ...
            try:
                shutil.copytree(EXMPL_D,
                                os.path.join(os.getcwd(), "mml_examples"))
            except OSError:
                sys.exit("mml_examples directory already exists")
            sys.exit("Created mml_examples directory")
        elif x == 'clean':
