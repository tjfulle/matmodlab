...
SUBROUTINE UMAT(STRESS,STATEV,DDSDDE,SSE,SPD,SCD, RPL,DDSDDT,DRPLDE,DRPLDT,&
  ...
  CHARACTER*8 :: CHARV(1)
  ! ---------------------------------------------------------------- UMAT --- !
  CALL HYPEREL(NPROPS-1, PROPS(2), TEMP, F1, NOEL, CMNAME, INCMPFLAG, &
       NSTATV, STATEV, NFLDV, FIELDV, DFIELDV, STRESS, DDSDDE)
  ...
  CASE (1)
     ! NUMERICAL JACOBIAN
     CALL JACOBIAN(NPROPS-1, PROPS(2), TEMP, NOEL, CMNAME, INCMPFLAG, &
          NSTATV, STATEV, NFLDV, FIELDV, DFIELDV, F1, DDSDDE)
  CASE (2)
     ALLOCATE(SNEO(6))
     CALL NEOHOOKE(2, (/PROPS(2),PROPS(3)/), F1, SNEO, DDSDDE)
     DEALLOCATE(SNEO)
  CASE (3)
     ...
     CALL NEOHOOKE(2, (/PROPS(2),PROPS(3)/), F1, SNEO, CNEO)
     ...
     ERR = SQRT(SUM((CNEO - DDSDDE)**2)) / MAX(MAXVAL(SNEO), ONE)
     IF (ERR > 1.E-10_DP) THEN
        WRITE(MSG, "(A,ES9.3)") "ERROR IN STIFFNESS: ", ERR
        CALL STDB_ABQERR(-1, MSG, INTV, REALV, CHARV)
     END IF

     DEALLOCATE(SNEO, CNEO)
  END SELECT
