...
class FileNotFoundError(Exception):
    def __init__(self, filename):
        ...
        if opts.debug: super(FileNotFoundError, self).__init__(message)
...
class ModelNotImportedError(Exception):
    def __init__(self, model):
        ...
        if opts.debug: super(ModelNotImportedError, self).__init__(message)
        else: raise SystemExit(message)
...
class ModelLibNotFoundError(Exception):
    def __init__(self, model):
        ...
        if opts.debug: super(ModelLibNotFoundError, self).__init__(message)
        else: raise SystemExit(message)
...
class DuplicateExtModule(Exception):
    def __init__(self, name):
        ...
        if opts.debug: super(DuplicateExtModule, self).__init__(message)
        else: raise SystemExit(message)
...
class GenericError(Exception):
    def __init__(self, message):
        ...
        if opts.debug: super(GenericError, self).__init__(message)
        else: raise SystemExit(message)
...
class UserInputError(Exception):
    def __init__(self, message):
        who = who_is_calling()
        message = adderr("{0}: ({1})".format(message.upper(), who))
        if opts.debug: super(InputError, self).__init__(message)
        else: raise SystemExit(message)
