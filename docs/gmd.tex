...
developing material models in Prof.~Brannon's \texttt{MED} driver (available
upon request from Prof.~Brannon). The \texttt{MED} driver was a special
purpose driver for driving material models through predefined strain paths.
After completing graduate school I began employment as a member of the
Technical Staff at Sandia National Labs. Among the many projects I worked on
was the development of material models for geologic applications. There, I
found need to drive the material models through prescribed stress paths to
match experimental records. This capability was not present in the
\texttt{MED} and I sought a different solution. The solution came from the
\texttt{mmd} driver, created years earlier at Sandia, by Tom Pucick. The
\texttt{mmd} driver had the capability to drive material models through
prescribed stress and strain paths, but also lacked many of the IO features of
the \texttt{MED}. And so, for some time I used both the \texttt{MED} and
\texttt{mmd} drivers in applications that suited their respective strengths.
After some time using both drivers, I decided to combine the best features of
each in to my own driver. Both the \texttt{MED} and \texttt{mmd} drivers were
written in Fortran and I decided to write the new driver in Python so that I
could leverage the large number of builtin libraries. The Numpy and Scipy
Python libraries would be used for handling most number crunching. The new
driver came to be known as \texttt{payette}. \texttt{payette} added many
unique capabilities and became a capable piece of software used by other staff
members at Sandia. But, \texttt{payette} suffered from the fact that it was my
first foray in to programming with Python. After some time, the bloat and bad
programming practices with \texttt{payette} caused me to spend a few weekends
re-writing it in to what is now known as \gmd{}.
...
% ----------------------------------------------------------------------------- %
\section{\gmd{} Solution Procedure}
\label{sec:gmdsol}
In addition to providing a platform for material model developers to formulate
and test constitutive routines, \gmd{} aims to provide users of material
...
solution procedure in \gmd{} is similar to that of the finite element method,
in that the host code (\gmd{}) provides to the constitutive routine a measure
...
each step and advancing kinematic quantities to the step's end, \gmd{}
...
are specified for a leg, \gmd{} internally determines the corresponding strain
components.  If a component of stress is specified, \gmd{} determines the
...
% ----------------------------------------------------------------------------- %
\chapter{Running Simulations with \gmd{}}
\label{chap:run}
\gmd{} is a command line driven programming.  To run a simulation with \gmd{},
be sure that \verb|$GMDROOT| is on your path and execute

\begin{verbatim}
$ gmd runid[.xml]
\end{verbatim}

where \texttt{runid} is the basename of the input file. Input file formatting
is covered in Chapters \ref{chap:inpov} - \ref{chap:inpopt}.

The following files will be produced by \gmd{} in the current working
directory
\begin{verbatim}
$ ls runid.*
