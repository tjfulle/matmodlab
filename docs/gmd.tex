...
\newcommand{\Example}[1]{\indent\colorbox{green}{%
...
\newcommand{\param}[2]{\ttnoindent{#1}\\%
  \indent\begin{minipage}{.95\textwidth}#2\end{minipage}}
\newcommand{\data}[3]{\ttnoindent{#1}.\textbf{#2}\\%
  \indent\begin{minipage}{.95\textwidth}#3\end{minipage}}
\newcommand{\class}[2]{\noindent\colorbox{orange}{%
    \begin{minipage}{\textwidth}%
      \emph{class} \texttt{#1}.\textbf{#2}%
    \end{minipage}}}
\newcommand{\method}[4]{\ttnoindent{#1}.\textbf{#2}(\emph{#3})\\%
  \indent\begin{minipage}{.95\textwidth}#4\end{minipage}}
...
\begin{itemize}
  ...
  \item \prompt{} \texttt{cd \$GMDROOT/toolset \&\& ./setup.py}
...
% --- interface API
\class{\underline{  }material}{Material}

% ----------------------------------------------------------------------------- %
\subsection{Class Data}
\data{Material}{name}{The material name. Name by which material is invoked
  from the input file.}\\[5pt]
\data{Material}{param\us{}names}{Ordered list of material
  parameter names as they appear in the input file.}\\[8pt]
%
The following is an example of a \texttt{Material} declaration.
\begin{example}
...
Called by Driver. Instantiate the material model. A material's
\texttt{\us\us{}init\us\us{}} method must call the
\texttt{\us{}\us{}init\us{}\us{}} method of the parent
class.\\[4pt]
%
\function{\texttt{Material}.\textbf{\us\us{}init\us\us{}(params)}}{%
  material = Material()}

The following is an example of a \texttt{\us\us{}init\us\us{}} method
\begin{example}
def __init__(self):
...
% ----------------------------------------------------------------------------- %
\subsection{Base Class Methods}

% ----------------------------------------------------------------------------- %
\subsection{Methods to be Defined by Each Material Model}
\subsubsection{Setup the Material}
Called by Driver. The method \texttt{setup} sets up the material
model by checking user input and requesting allocation of storage of material
variables.\\[4pt]
%
\function{\texttt{Material}.\textbf{setup(params)}}{%
    material.setup(params)}\\[4pt]
\param{ndarray params}{Material parameters parsed from the input file}\\[4pt]
%
...
\subsubsection{Adjusting the Initial State}
Called by Driver. The method \texttt{adjust\us{}initial\us{}state}
adjusts the initial state after the material is setup.  Method provided by
base class should be adequate for most materials.  A material should only
overide the base method if absolutely necessary. \\[4pt]
%
\function{\texttt{Material}.\textbf{adjust\us{}initial\us{}state(xtra)}}{%
    material.adjust\us{}initial\us{}state(xtra)}

\subsubsection{Update the Material State}
Called by Driver. The material state is updated to the end of the step via
the \texttt{update\us{}state} method. Each material model must provide its own
\texttt{update\us{}state} method.\\[4pt]
%
\function{\texttt{Material}.\textbf{update\us{}state(dt, d, sig, xtra, %
    *args, **kwargs)}}{%
  stress, xtra = material.update\us{}state(dt, d, sig, xtra, *args,
  **kwargs)}\\[8pt]
\param{dt real}{timestep size}\\
...
\param{dict kwargs}{extra keyword args (not used)}\\[8pt]
...
% --- makemf API
\function{\texttt{makemf}.\textbf{makemf}}{%
