...
""".format(self.runid, self.driver.kind, self.driver.num_leg,
    ...
    def write_input(self):
        ...
        filename = self.runid + ".py"
        if os.path.isfile(filename):
            for i in range(9):
                filename = "{0}_{1}.py".format(self.runid, i+1)
                if not os.path.isfile(filename):
                    break
            else:
                self.logger.error("unable to determine unique "
                                  "input file name to write")
                return
        with open(filename, "w") as fh:
            fh.write("# Input file automatically generated by matmodlab\n")
            fh.write("from matmodlab import *\n")
            fh.write("mps = MaterialPointSimulator('{0}')\n".format(self.runid))
            fh.write(self.driver.tostr(obj="mps"))
            fh.write(self.material.tostr(obj="mps"))
            fh.write("mps.run()\n")
